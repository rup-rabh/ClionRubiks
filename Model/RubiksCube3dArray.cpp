//
// Created by rupra on 28-06-2024.
//
	...
	private:
		...
		void rotateFace(int ind)
		{
			...
			for (int i = 0 ; i < 3 ; i++)
			{
				for (int j = 0 ; j < 3 ; j++)
				{
					tempArr[i][j]=cube[ind][i][j];
				}
			}
			for (int i = 0; i < 3; i++) cube[ind][0][i] = tempArr[2 - i][0];
			for (int i = 0; i < 3; i++) cube[ind][i][2] = tempArr[0][i];
			for (int i = 0; i < 3; i++) cube[ind][2][i] = tempArr[2-i][2];
			for (int i = 0; i < 3; i++) cube[ind][i][0] = tempArr[2][i];
		}
public:
	char cube[6][3][3] {};
	...
	bool operator==(const RubiksCube3dArray &r1) const { //equality operator
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 3; j++) {
				for (int k = 0; k < 3; k++) {
	...
	RubiksCube3dArray &operator=(const RubiksCube3dArray &r1) { //assignmment operator
		for (int i = 0; i < 6; i++) {
			for (int j = 0; j < 3; j++) {
				for (int k = 0; k < 3; k++) {
	...
	struct Hash3d {
		size_t operator()(const RubiksCube3dArray &r1) const { //hash Rule for cube
			string str = "";
			for (int i = 0; i < 6; i++) {
				for (int j = 0; j < 3; j++) {
					for (int k = 0; k < 3; k++) {
